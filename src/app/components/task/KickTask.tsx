...
interface KickTaskProps {
  isOpen: boolean;
  toggleOpen: () => void;
}

const KickTask: React.FC<KickTaskProps> = ({ isOpen, toggleOpen }) => {
  ...
  return (
    <div className="bg-white overflow-hidden w-full flex flex-col border 4 border-[#53FC18] rounded-[12px]">
      <div 
        className="bg-[#53FC18] p-3 flex justify-between items-center cursor-pointer"
        onClick={toggleOpen}
      >
        ...
        <div className="flex items-center space-x-2">
          ...
          {isOpen ? <FaChevronUp className="md:hidden text-black" /> : <FaChevronDown className="md:hidden text-black" />}
        </div>
      </div>
