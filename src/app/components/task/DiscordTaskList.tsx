...
import { FaDiscord, FaChevronDown, FaChevronUp } from 'react-icons/fa';
...
interface DiscordTaskListProps {
  isOpen: boolean;
  toggleOpen: () => void;
}
...
const DiscordTaskList: React.FC<DiscordTaskListProps> = ({ isOpen, toggleOpen }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden w-full flex flex-col h-full border 4 border-[#5865F2]">
      <div 
        className="bg-[#5865F2] text-white p-3 flex justify-between items-center cursor-pointer"
      ...
      >
        ...
        <div className="flex items-center space-x-2">
          ...
          {isOpenState ? <FaChevronUp className="md:hidden" /> : <FaChevronDown className="md:hidden" />}
        </div>
      </div>
      <div className={`bg-white flex-grow overflow-y-auto ${isOpenState ? 'block' : 'hidden md:block'}`}>
        {tasks.map((task) => (
          <Task
            key={task.id}
            {...task}
